...
class MostPopular extends Component {
  ...
  componentWillReceiveProps(nextProps){
    if(nextProps.tvShows.message ==='Not Found'){
      ...
      this.setState({
        isTVReady: true,
        tvShows1: nextProps.tvShows.tvShows.results.slice(0,6),
        tvShows2: nextProps.tvShows.tvShows.results.slice(6,12),
        tvShows3: nextProps.tvShows.tvShows.results.slice(12,18)
      })
    }
    ...
    if(nextProps.movies.message ==='Not Found'){
      ...
      this.setState({
        isMoviesReady: true,
        movies1: nextProps.movies.movies.results.slice(0,6),
        movies2: nextProps.movies.movies.results.slice(6,12),
        movies3: nextProps.movies.movies.results.slice(12,18)
      })
    }
  }

  render() {
    const { isTVReady, isMoviesReady, isXsScreen, tvShow, show, videos, movie, isMovies } = this.state;
              ...
              (
                <Carousel>
                  ...
                  {  isTVReady  && this.state.tvShows1.map( (tvShow,i) =>
                    <Col  style={{padding:10}} key={tvShow.id} xs={12} sm={4}  lg={2}>
                      <Button style={{width:'100%'}} onClick={() => this.handleShow(tvShow.id, false)} >
                        <h6>{tvShow.original_name}</h6>
                        {tvShow.poster_path !== null ?
                          <img
              ...
              (
                <Carousel>
                  ...
                  {  isMoviesReady  && this.state.movies1.map( (movie,i) =>
                    <Col  style={{padding:10}} key={movie.id} xs={12} sm={4} lg={2}>
                      <Button style={{width:'100%'}} onClick={() => this.handleShow(movie.id, true)} >
                        <h6>{movie.title}</h6>
                        {movie.poster_path !== null ?
                          <img
